        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {formulas.map((item, idx) => (
            <Flashcard
              ...
              isCopied={copiedCardIndex === idx}
              appExample={getApplicationExample(item.term, item.definition)}
              onToggle={toggleCard}
              onCopy={copyCard}
              academicIcon={getAcademicIcon(idx)}
            />
          ))}
        </div>
      </section>
...
export default React.memo(RevisionPack);
